...
// static 으로 선언된 변수, 메서드 객체와는 상관없이 호출할 수 있다는 의미, 따라서 객체를 생성할 필요 없이 클래스 이름만으로도 사용할 수 있다.
// 예를 들면 Math 클래스는 대부분 메서드 static으로 선언했기때문에 간단한 계산할 때 매번 객체를 생성할 필요없이 바로 사용가능하다. abs(절대값) static으로 선언이 되어 있기 때문에 int x = Math.abs(x);
// Math m = math();
// int x = m.abs(x);

// 1. static 메서드는 오직 static 변수만 사용할수 있다. 
// 2. static 메서드는 오직 같은 클래스 내에서의 static 메서드만 호출할 수 있다. 
// 3. static 메서드는 this 사용할수 없다. 
